                            ...
                            <div class="main-content-wrap">
                                <div class="flex items-center flex-wrap justify-between gap20 mb-27">
                                    ...
                                    <ul class="breadcrumbs flex items-center flex-wrap justify-start gap10">
                                        ...
                                        <li>
                                            <div class="text-tiny">New FeeType</div>
                                        </li>
                                    </ul>
                                </div>
                                <!-- new-category -->
                                <div class="wg-box">
                                    <form class="form-new-product form-style-1" action="{{route('admin.feetype.store')}}" method="POST">
                                        @csrf
                                        ...
                                        <fieldset class="category">
                                            ...
                                            <select id="teacher_id" class="flex-grow" name="fee_type_id">
                                                ...
                                                @foreach($fees as $fee)
                                                    <option value="{{ $fee->id }}" {{ old('fee_type_id') == $fee->id ? 'selected' : '' }}>
                                                        {{ $fee->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <fieldset class="name">
                                            ...
                                            <input class="flex-grow" type="text" placeholder="Amount" name="amount"
                                                tabindex="0" value="{{old('amount')}}" aria-required="true" required="">
                                        </fieldset>
                                        @error('amount')
                                        <span class="alert alert-danger text-center" role="alert">{{ $message }}</span>      
                                        @enderror
                                        <fieldset class="name">
                                            ...
                                            <input class="flex-grow" type="date" placeholder="Payment Date " name="payment_date"
                                                tabindex="0" value="{{old('payment_date')}}" aria-required="true" required="">
                                        </fieldset>
                                        @error('payment_date')
                                        <span class="alert alert-danger text-center" role="alert">{{ $message }}</span>      
                                        @enderror
